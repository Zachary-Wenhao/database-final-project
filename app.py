...
# Developer Local Laptop Configuration
# uri = "mongodb+srv://atlasxu:200286xwhWH@cluster0.rladyyl.mongodb.net/?retryWrites=true&w=majority"
# connection = pymongo.MongoClient(uri)
# db = connection['sj3251'] #'sj3251'
# set up the routes
...
def search():
    ...
    docs = db.exampleapp.find({"$or":[{"name": {"$regex": query}},{"topic": {"$regex": query}},{"message": {"$regex": query}}]}).sort("created_at", -1) # sort in descending order of created_at timestamp
    return render_template('search_results.html', docs=docs, page_title = query)
...
@app.route('/reply/<mongoid>')
def reply(mongoid):
    """
    Route for GET requests to the reply page.
    Displays a form users can fill out to edit an existing record.
    """
    doc = db.exampleapp.find_one({"_id": ObjectId(mongoid)})
    return render_template('reply.html', mongoid=mongoid, doc=doc) # render the edit template


@app.route('/reply/<mongoid>', methods=['POST'])
def reply_post(mongoid):
    """
    Route for POST requests to the reply page.
    Accepts the form submission data for the specified document and updates the document in the database.
    """
    name = request.form['fname']
